...
from seasonalweather.jobs.worker_client import SynthesisClient, WorkerSynthesisClient
...
from seasonalweather.tts.remote_client import RemoteSynthesisClient

from .models import ReloadDiff, ReloadDisposition
from .safe_point import ActivityRegistry
from .safe_point import TTS as RELOAD_TTS_ACTIVITY
...
class OrchestratorPreparedResources:
    ...
    synthesizer: SynthesisClient | None
    ...
    @property
    def tts(self) -> SynthesisClient | None:
        """Compatibility view over the controller's bounded synthesis port."""
    ...
    async def retire(self) -> None:
        ...
        if client is not None:
            await client.aclose()
        old_synthesizer = next(
            (value for owner, name, value in self._snapshot if owner is self.orch and name == "synthesizer"),
            None,
        )
        self._close_synthesizer(old_synthesizer)

    @staticmethod
    def _close_synthesizer(synthesizer: object) -> None:
        close_synthesizer = getattr(synthesizer, "close", None)
    ...
    def _orchestrator_assignments(self) -> tuple[tuple[Any, str, object], ...]:
        ...
        if self.plan.replace_tts and self.synthesizer is not None:
            if hasattr(self.orch, "synthesizer"):
                items.append((self.orch, "synthesizer", self.synthesizer))
            if hasattr(self.orch, "tts"):
                items.append((self.orch, "tts", self.synthesizer))
    ...
    def _dependent_assignments(self) -> tuple[tuple[Any, str, object], ...]:
        cfg = self.configuration
        items: list[tuple[Any, str, object]] = []
        formatters = getattr(self.orch, "formatters", None)
        cap_text = getattr(formatters, "cap", None)
    ...
    def _refresher_assignments(self) -> tuple[tuple[Any, str, object], ...]:
        ...
        items: list[tuple[Any, str, object]] = [
            live_fields[path] for path in self.plan.changed_paths if path in live_fields
        ]
        if self.plan.replace_cycle_builder:
            items.extend(
                (
        ...
        if self.plan.replace_tts:
            refresher_field = "_synthesizer" if hasattr(refresher, "_synthesizer") else "_tts"
            items.append((refresher, refresher_field, self.synthesizer))
        if self.plan.replace_timezone:
    ...
    def _conductor_assignments(self) -> tuple[tuple[Any, str, object], ...]:
        ...
        if conductor is not None:
            ...
            if self.plan.replace_tts:
                items.append((conductor, "_synthesizer", self.synthesizer))
            if self.plan.replace_timezone:
...
class OrchestratorResourcePreparer:
    ...
    async def prepare(
        ...
        *,
        ...
        active_synthesizer = getattr(self.orch, "synthesizer", getattr(self.orch, "tts", None))
        replacement_synthesizer: SynthesisClient | None = None
        if resource_plan.replace_tts:
            if configuration.tts.backend == "local":
                replacement_synthesizer = WorkerSynthesisClient(
                    configuration=configuration,
                    input_root=(
                        active_synthesizer.input_store.root
                        if isinstance(active_synthesizer, WorkerSynthesisClient)
                        else Path(configuration.paths.artifact_dir)
                        / "worker-artifacts"
                        / "staging"
                        / "controller-inputs"
                    ),
                ...
                if isinstance(active_synthesizer, WorkerSynthesisClient):
                    replacement_synthesizer._job_service = active_synthesizer._job_service
                    replacement_synthesizer._repository = active_synthesizer._repository
                    replacement_synthesizer._active_root = active_synthesizer._active_root
                    replacement_synthesizer._bound = active_synthesizer._bound
                else:
                    job_service = getattr(self.orch, "worker_job_service", None)
                    repository = getattr(self.orch, "worker_repository", None)
                    active_root = getattr(self.orch, "worker_active_root", None)
                    if job_service is not None and repository is not None and active_root is not None:
                        replacement_synthesizer.bind(
                ...
                replacement_synthesizer = RemoteSynthesisClient(
                    configuration=configuration,
                    admission_check=lambda: self.orch.lifecycle.require(WorkClass.TTS),
                    activity_context=lambda: self.activities.activity(RELOAD_TTS_ACTIVITY),
                )
            replacement_synthesizer.configuration_generation = target_generation
        plan = OrchestratorPreparedResources(
            orch=self.orch,
            plan=resource_plan,
            configuration=configuration,
            timezone=timezone,
            synthesizer=replacement_synthesizer if resource_plan.replace_tts else None,
    ...
    def synchronize_generation(self, generation: int) -> None:
        ...
        synthesizer = getattr(self.orch, "synthesizer", getattr(self.orch, "tts", None))
        if synthesizer is not None and hasattr(synthesizer, "configuration_generation"):
            synthesizer.configuration_generation = generation
